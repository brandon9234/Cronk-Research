...
function initialDashboardView() {
  ...
  if (!VISIBLE_DASHBOARD_VIEWS.has(viewId)) return "";
...
async function activateView(viewId, options = {}) {
  if (!VISIBLE_DASHBOARD_VIEWS.has(viewId)) viewId = DEFAULT_DASHBOARD_VIEW;
  const view = document.getElementById(viewId);
  if (!view) return;
  if (viewId !== "listings") {
    cancelReviewListingSearch();
...
async function boot() {
  ...
  renderAll();
  const initialView = initialDashboardView() || DEFAULT_DASHBOARD_VIEW;
